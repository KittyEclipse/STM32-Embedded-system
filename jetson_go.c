#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
#include <string.h>
#include <termios.h>
#include <unistd.h>
#include <sys/select.h>
#include <time.h>

static int set_serial(int fd, int baud)
{
  struct termios tty;
  if (tcgetattr(fd, &tty) != 0) {
    perror("tcgetattr");
    return -1;
  }

  cfmakeraw(&tty);

  tty.c_cflag &= ~PARENB;
  tty.c_cflag &= ~CSTOPB;
  tty.c_cflag &= ~CSIZE;
  tty.c_cflag |= CS8;

  tty.c_cflag |= (CLOCAL | CREAD);
  tty.c_cflag &= ~CRTSCTS;

  tty.c_cc[VMIN]  = 0;
  tty.c_cc[VTIME] = 0;

  speed_t spd = B115200;
  (void)baud;
  cfsetispeed(&tty, spd);
  cfsetospeed(&tty, spd);

  if (tcsetattr(fd, TCSANOW, &tty) != 0) {
    perror("tcsetattr");
    return -1;
  }

  tcflush(fd, TCIOFLUSH);
  return 0;
}

static int write_all(int fd, const char *s)
{
  size_t n = strlen(s);
  while (n) {
    ssize_t w = write(fd, s, n);
    if (w < 0) {
      if (errno == EINTR) continue;
      perror("write");
      return -1;
    }
    s += (size_t)w;
    n -= (size_t)w;
  }
  return 0;
}

static int send_line(int fd, const char *line_no_nl)
{
  char buf[128];
  snprintf(buf, sizeof(buf), "%s\n", line_no_nl);
  return write_all(fd, buf);
}

static long long now_ms(void)
{
  struct timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  return (long long)ts.tv_sec * 1000LL + (long long)ts.tv_nsec / 1000000LL;
}

static int drain_uart_print_lines(int fd, const char *needle)
{
  static char acc[1024];
  static size_t acc_len = 0;

  char rxbuf[256];
  int saw = 0;

  for (;;) {
    ssize_t n = read(fd, rxbuf, sizeof(rxbuf));
    if (n <= 0) break;

    for (ssize_t i = 0; i < n; i++) {
      char c = rxbuf[i];
      if (acc_len < sizeof(acc) - 1) {
        acc[acc_len++] = c;
      }

      if (c == '\n' || c == '\r') {
        while (acc_len && (acc[acc_len - 1] == '\n' || acc[acc_len - 1] == '\r')) acc_len--;
        acc[acc_len] = '\0';

        if (acc_len) {
          printf("STM32: %s\n", acc);
          fflush(stdout);
          if (needle && needle[0] && strstr(acc, needle)) {
            saw = 1;
          }
        }

        acc_len = 0;
      }
    }
  }

  return saw;
}

static int wait_for_token(int fd, const char *needle, int timeout_ms)
{
  long long t_end = now_ms() + (long long)timeout_ms;

  while (now_ms() < t_end) {
    fd_set rset;
    FD_ZERO(&rset);
    FD_SET(fd, &rset);

    struct timeval tv;
    tv.tv_sec = 0;
    tv.tv_usec = 200000;

    int r = select(fd + 1, &rset, NULL, NULL, &tv);
    if (r < 0) {
      if (errno == EINTR) continue;
      perror("select");
      return 0;
    }

    if (FD_ISSET(fd, &rset)) {
      if (drain_uart_print_lines(fd, needle)) return 1;
    }
  }
  return 0;
}

int main(void)
{
  const char *dev = "/dev/ttyTHS1";
  int fd = open(dev, O_RDWR | O_NOCTTY | O_NONBLOCK);
  if (fd < 0) {
    perror("open /dev/ttyTHS1");
    return 1;
  }

  if (set_serial(fd, 115200) != 0) {
    close(fd);
    return 1;
  }

  printf("Opened %s @115200 8N1\n", dev);
  printf("Commands:\n");
  printf("  g = GO    (start walking, waits for STM32 confirm)\n");
  printf("  s = STOP  (disable outputs, return to neutral)\n");
  printf("  f = FWD   (switch to forward while running)\n");
  printf("  b = BACK  (switch to backward while running)\n");
  printf("  h = HELP  (ask STM32 to list valid commands)\n");
  printf("  q = quit\n\n");

  char inbuf[64];

  while (1) {
    fd_set rset;
    FD_ZERO(&rset);
    FD_SET(fd, &rset);
    FD_SET(STDIN_FILENO, &rset);

    int maxfd = (fd > STDIN_FILENO) ? fd : STDIN_FILENO;

    struct timeval tv;
    tv.tv_sec = 0;
    tv.tv_usec = 200000;

    int r = select(maxfd + 1, &rset, NULL, NULL, &tv);
    if (r < 0) {
      if (errno == EINTR) continue;
      perror("select");
      break;
    }

    // Always drain any incoming STM32 data first
    if (FD_ISSET(fd, &rset)) {
      (void)drain_uart_print_lines(fd, NULL);
    }

    if (FD_ISSET(STDIN_FILENO, &rset)) {
      if (!fgets(inbuf, sizeof(inbuf), stdin)) break;

      // Strip trailing newline / carriage return
      inbuf[strcspn(inbuf, "\r\n")] = '\0';

      // Silently ignore blank lines (bare Enter)
      if (inbuf[0] == '\0') continue;

      // Reject anything longer than a single character (arrow keys, paste, etc.)
      if (inbuf[1] != '\0') {
        printf("Unknown input: \"%s\" | Use single keys: g, s, f, b, h, q\n\n", inbuf);
        continue;
      }

      if (inbuf[0] == 'q') {
        printf("Quitting.\n");
        break;
      }
      else if (inbuf[0] == 'g') {
        if (send_line(fd, "GO") != 0) break;
        printf("Sent: GO (waiting for STM32 confirm...)\n");
        if (wait_for_token(fd, "CONFIRMED. STARTING WALK.", 3000)) {
          printf("Confirm: STM32 accepted GO\n\n");
        } else {
          printf("No confirm received within 3s. Check wiring/baud or STM32 output.\n\n");
        }
      }
      else if (inbuf[0] == 's') {
        if (send_line(fd, "STOP") != 0) break;
        printf("Sent: STOP (STM32 should disable outputs and return neutral)\n\n");
      }
      else if (inbuf[0] == 'f') {
        if (send_line(fd, "FWD") != 0) break;
        printf("Sent: FWD\n\n");
      }
      else if (inbuf[0] == 'b') {
        if (send_line(fd, "BACK") != 0) break;
        printf("Sent: BACK\n\n");
      }
      else if (inbuf[0] == 'h') {
        if (send_line(fd, "HELP") != 0) break;
        printf("Sent: HELP\n\n");
      }
      else {
        printf("Unknown key: '%c' | Valid keys: g, s, f, b, h, q\n\n", inbuf[0]);
      }
    }
  }

  close(fd);
  return 0;
}
